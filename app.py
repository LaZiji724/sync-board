...
monkey.patch_all() # 必须在最顶行，确保所有库都被异步化
...
app = Flask(__name__)
app.config['SECRET_KEY'] = 'laziji-gevent-v16'
# 切换为 gevent 模式
...
<head>
    ...
    <style>
        ...
        .btn-add:active, .btn-clear:active { transform: scale(0.95); opacity: 0.8; }
        ...
        textarea { width: 100%; height: 160px; border: 1px solid #ddd; border-radius: 10px; padding: 14px; font-size: 16px; box-sizing: border-box; outline: none; background: #fafafa; line-height: 1.6; transition: border-color 0.2s, background 0.2s; }
        textarea:focus { border-color: #2a9d8f; background: white; box-shadow: 0 0 0 3px rgba(42, 157, 143, 0.1); }
...
<body>
    <div class="container">
        <div class="toolbar">
            ...
            <button class="btn-clear" onclick="handleGlobalClear()">🗑️ 清除可见内容</button>
        </div>
        <div id="box-container"></div>
    </div>
    <div class="footer">
        <span style="font-size:12px; color:#999; white-space:nowrap;">👥 在线私聊：</span>
    ...
    <script>
        ...
        while(!myName || myName.trim() === "") { myName = prompt("请输入你的名字 (建议用中文):"); }
        ...
        socket.on('sync_structure', (data) => {
            ...
            for (let key in data.whispers) {
                if (key.includes(myName)) {
                    ...
                    createBox(container, `🤐 与 ${other} 的私密同步`, 'whi', other, data.whispers[key]);
                }
            }
        });
        ...
        function createBox(container, title, type, id, content) {
            ...
            div.innerHTML = `
                ...
                <button style="background:#eee; color:#666;" onclick="${clearCmd}">清空</button></div>
                <textarea id="${type}-${id}" 
        ...
        function handleGlobalClear() {
            if(confirm('确定清空你能看到的所有公共区和私聊吗？')) socket.emit('manage_box', {action: 'clear_visible_all'});
        }
        ...
        socket.on('update_content', (data) => {
            ...
            // 核心逻辑：如果是清屏指令（内容为空），无视状态强制清空
            const isClearCmd = (data.text === "");
            if (!isClearCmd) {
                // 如果我正在打字或刚打完 800ms 内，拒绝覆盖
