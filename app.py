...
app = Flask(__name__)
app.config['SECRET_KEY'] = 'laziji-eventlet-final'
# 回归 eventlet 模式
...
<head>
    ...
    <style>
        ...
        .footer { position: fixed; bottom: 0; left: 0; right: 0; background: rgba(255,255,255,0.95); padding: 15px 20px; border-top: 1px solid #eee; display: flex; align-items: center; gap: 10px; overflow-x: auto; z-index: 1000; }
...
<body>
    <div class="container">
        <div class="toolbar">
            ...
            <button class="btn-clear" onclick="handleGlobalClear()">🗑️ 清除内容</button>
        </div>
    ...
    <script>
        ...
        function createBox(container, title, type, id, content) {
            ...
            div.innerHTML = `<div class="card-title"><span>${title}</span><button onclick="${clearCmd}">清空</button></div>
        ...
        function handleGlobalClear() { if(confirm('确定清空吗？')) socket.emit('manage_box', {action: 'clear_visible_all'}); }
...
def handle_whisper(data):
    me = state.get("users", {}).get(request.sid, {}).get('name')
    if not me: return
    target = data['target']
    key = "|".join(sorted([me, target]))
    state["whispers"][key] = data['text']
    for sid, info in state["users"].items():
        if info['name'] == target: socketio.emit('update_content', {'type': 'whi', 'id': me, 'text': data['text']}, room=sid)
        elif info['name'] == me: socketio.emit('update_content', {'type': 'whi', 'id': target, 'text': data['text']}, room=sid)
